...
UINT8 ucGetKeyValue(void)
{
   ...
   if(0x55==_KBC_INT_FLAG_)//在中断被置起
   {
		...
		{		     
			 ...
			 _IOM_FLAG_=0x00;			 
			 if(0x00!=(KeyTmp[0]))									//第一次是列中断。第二次是行中断
			 {		     
		...
		{			 
			// vTimer0_Stop();
			 ...
			 if(_TIMER_FLAG_>=LONG_PRESS_TIME)	 //长按
			 {
			      _TIMER_FLAG_=0;
				  for(i=0;i<16;i++)
				  {
   ...
   else
   {
        //超时处理，按键一直没有释放，关定时器，给出常按键值
		 if(_TIMER_FLAG_>=PRESS_TIME_OUT)	 //长按超时
		 {
